...
public class Backend implements Comparable<Backend>{
	public static final int DISK_BLOCK_SIZE = 8;
	...
	//public static final int MAX_CAPACITY = 7200000; //GB 800*3TB SATA 7200rpm RAID6
	//public static final int MAX_BANDWIDTH = 15584; // 0.7 read / 0.3 write / 8k block
...
//	public static int CAP_DOWN_TH = (int) (DOWN_TH * MAX_CAPACITY);
	...
	public Backend() {
		id = idGen++;
		this.MAX_CAPACITY = 7200000;
		this.MAX_BANDWIDTH = 15584;
	...
	public Backend(int capacity, int bandwidth) {
		id = idGen++;
        this.MAX_CAPACITY = capacity;
        this.MAX_BANDWIDTH = bandwidth;
		this.allocatedCapacity = 0;
		this.currentThroughput = 0;
		this.volumeSpeed = 0;
		this.availableVolumeSpeed = MAX_BANDWIDTH;
...
//			availableVolumeSpeed = currentThroughput / (numOfIO + numOfMigrating + outMigNum + 1) ;
			...
			int totalConsumedBandwitdh = 0;
			for(Volume volume : volumes) {
				totalConsumedBandwitdh += volume.getSLA();
			}

			if (totalConsumedBandwitdh <= MAX_BANDWIDTH){
				for(Volume volume : volumes) {
					volume.setSpeed(volume.getSLA());
				}
				availableVolumeSpeed = MAX_BANDWIDTH - totalConsumedBandwitdh;
				volumeSpeed = MAX_BANDWIDTH - availableVolumeSpeed;
			}else{
				int extraBandwitdh = (totalConsumedBandwitdh - MAX_BANDWIDTH) / volumes.size();
				for(Volume volume : volumes) {
					volume.setSpeed(volume.getSLA() - extraBandwitdh);
				}

				availableVolumeSpeed = 0;
				volumeSpeed = MAX_BANDWIDTH;
			}
...
/*		} else {
	...
	public int calWeight() {
		if (VolumeRequest.DIMENTION == 1) {
			return this.MAX_CAPACITY - allocatedCapacity;
		} else if (VolumeRequest.DIMENTION == 2) {
			return (int)(100 * ((double)(this.MAX_CAPACITY - allocatedCapacity) / this.MAX_CAPACITY + (double)availableVolumeSpeed / this.MAX_BANDWIDTH));
		} else {
			return 0;
		}
	}
